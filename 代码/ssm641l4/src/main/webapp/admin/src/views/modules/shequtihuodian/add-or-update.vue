<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="提货地点" prop="tihuodidian">
          <el-input v-model="ruleForm.tihuodidian" 
              placeholder="提货地点" clearable  :readonly="ro.tihuodidian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="提货地点" prop="tihuodidian">
              <el-input v-model="ruleForm.tihuodidian" 
                placeholder="提货地点" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='tihuodidian'){
            this.ruleForm.tihuodidian = obj[o];
	    this.ro.tihuodidian = true;
            continue;
          }
        }
      }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shequtihuodian/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shequtihuodianCrossAddOrUpdateFlag = false;
                  this.parent.search();
